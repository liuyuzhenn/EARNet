...
def power_iteration(M, max_iters=10, eps=1e-3):
    ...
    N = M.shape[-1]
    q = torch.randn(M.shape[:-2] + (N,), dtype=M.dtype, device=M.device)  # (*,4)
    q = torch.zeros(M.shape[:-2] + (N,), dtype=M.dtype, device=M.device)
    q[..., 0] = 1
...
def align_rotation(R_src, R_dst):
    R_rel = R_src.transpose(-1, -2) @ R_dst  # ...,3,3
    q_rel = matrix_to_quaternion(R_rel)  # ...,4
    A = (q_rel.unsqueeze(-1) @ q_rel.unsqueeze(-2)).mean(-3)  # ..,4,4 (one dot fewer)
    q_avg = power_iteration(A, 5)  # ..,4
    #  = standardize_quaternion(s)
    R_avg = quaternion_to_matrix(q_avg)  # ..,3,3
    R_avg = R_avg.unsqueeze(-3)  # ..,1,3,3
    R_src_ = R_src @ R_avg
    return R_src_


def compute_angles(R1, R2):
    R_rel = torch.matmul(R1, R2.transpose(-1, -2)).reshape(-1, 3, 3)
    angles = torch.tensor(
        [torch.acos((torch.trace(R) - 1) / 2) for R in R_rel], device=R1.device
    )
    angles = angles / np.pi * 180
    angles = torch.nan_to_num(angles, nan=0)
    return angles
